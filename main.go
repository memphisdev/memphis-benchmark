		...
		for {
			if done {
				...
				case <-time.After(time.Second * 1):
					...
					command := fmt.Sprintf("nats stream info '%s' --server=%s:6666 --user=%s --password=%s", stationName+"$1", host, username, pass)
					...
					if opType == "e2e" || opType == "consume" { // e2e - count based on the consumed messages
						command := fmt.Sprintf("nats consumer info '%s' group1 --server=%s:6666 --user=%s --password=%s", stationName+"$1", host, username, pass)
						cmd := exec.Command("bash", "-c", command)
						var outb bytes.Buffer
						cmd.Stdout = &outb
						err = cmd.Run()
					...
					command = fmt.Sprintf("nats stream purge '%s' -f --server=%s:6666 --user=%s --password=%s", stationName+"$1", host, username, pass)
					cmd = exec.Command("bash", "-c", command)
					err := cmd.Run()
					if err != nil {
						cmd = exec.Command("sh", "-c", command)
